...
public class TransactionController {
    ...
    public ResponseEntity<Object> newCard(Authentication authentication, @RequestParam Double amount, @RequestParam String description, @RequestParam String originAccount, @RequestParam String destinationAccount){
        ...
        Transaction fromTransaction = new Transaction(DEBIT, -amount, description + " to " + destinationAccount, LocalDateTime.now(), fromAccount.getBalance()-amount,true);
        fromAccount.addTransaction(fromTransaction);
        fromAccount.setBalance(fromAccount.getBalance() - amount);
        transactionService.saveTransaction(fromTransaction);


        Transaction toTransaction = new Transaction(CREDIT, amount, description + "from" + originAccount, LocalDateTime.now(),toAccount.getBalance()+amount, true);
